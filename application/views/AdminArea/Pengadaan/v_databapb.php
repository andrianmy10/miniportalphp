...
<div class="row fade-in-up">
    <div class="col-sm-12">
        ...
        <div class="pb-2 mb-3">
            <h4 class="mb-0 fw-bold" style="font-size: 18px;">Master Data BAPB</h4>
        </div>
    </div>
</div>
...
<!-- Tambahin overflow: hidden !important; biar Progress Bar-nya gak bablas -->
<div class="card-sp3 fade-in-up delay-2" style="position: relative; z-index: 1; min-height: 400px; overflow: hidden !important;">
    <div class="table-responsive pt-2" style="overflow-x: visible;">
        <table id="tableBapb" class="table table-hover display nowrap" style="width:100%">
            <thead>
                <tr>
                    <th>NO BAPB</th>
                    <th class="text-center">TANGGAL BAPB</th>
...
<script>
    $(document).ready(function() {
        ...
        function getFileName() { return 'Data_BAPB_' + $('#tgl_awal').val() + '_sd_' + $('#tgl_akhir').val(); }

        var table = $('#tableBapb').DataTable({
            "processing": true, // <--- WAJIB TRUE BUAT PROGRESS BAR
            "serverSide": false,
            "scrollX": true,
            "order": [[ 1, "desc" ]], // Default sort by Tanggal BAPB Terbaru
                ...
                { 
                    ...
                    "render": function(data, type, row) {
                        if (type === 'display' && data) {
                            let nobapb = encodeURIComponent(data);
                            let urlBirt = `https://birt.asihputera.or.id:9999/birt/frameset?__report=Pengadaanbapb.rptdesign&Group=manajemen&Departemen=MI%20ASIH%20PUTERA&nobapb=${nobapb}`;
                            
                            return `<a href="${urlBirt}" target="_blank" class="fw-bold text-primary text-decoration-none" title="Buka Report BAPB"><i class="fa-solid fa-arrow-up-right-from-square me-1 text-primary"></i> ${data}</a>`;
                        }
                        return data;
                    }
                },
            ...
            "buttons": [
                ...
                { 
                    ...
                    customize: function (doc) {
                        doc.content.splice(0, 1, { text: 'DATA BAPB', fontSize: 16, bold: true, alignment: 'center', color: '#1e3a8a', margin: [0, 0, 0, 5] });
            ...
            "language": { 
                "search": "", 
                "searchPlaceholder": "Cari No BAPB, Vendor...",
                "processing": "" // <--- KOSONGIN BIAR PROGRESS BAR GAK KETIMPA TEKS
            },
            ...
            "ajax": function (data, callback, settings) {
                ...
                let cacheKey = 'cache_databapb_' + tgl_awal + '_' + tgl_akhir;
                let cachedData = sessionStorage.getItem(cacheKey);

                if (cachedData) {
                    console.log("Ambil data BAPB dari Cache Browser");
                    callback(JSON.parse(cachedData));
                } else {
                    console.log("Ambil data BAPB dari Server");
                    $.ajax({
                        url: "<?= base_url('pengadaan/ajax_data_bapb') ?>",
                        type: "POST",
                        data: {
                            tgl_awal: tgl_awal,
                            tgl_akhir: tgl_akhir
        ...
        $('#btn-filter').click(function(){ 
            let tgl_awal = $('#tgl_awal').val();
            let tgl_akhir = $('#tgl_akhir').val();
            sessionStorage.removeItem('cache_databapb_' + tgl_awal + '_' + tgl_akhir);
            table.ajax.reload(); 
        });

        $('#btn-reset').click(function(){
            Object.keys(sessionStorage).forEach(function(key){
               if(key.startsWith('cache_databapb_')) {
